...
enum EpisodePromptBuilder {
    ...
    - **가장 중요**: 관찰 캡션에 사람의 외모·옷차림·표정·행동이 묘사돼도, 출연 정보가 '이름 모를 출연자'이면
      그 묘사는 전부 그 출연자의 것이다. 절대 주인공의 모습·행동으로 옮겨 적지 마라.
      주인공은 그 사람을 카메라에 담은 촬영자다.
      (예: 캡션 "회색 티셔츠를 입은 여성이 웃고 있다" + 출연 "이름 모를 출연자 1명"
       → 금지: "승보는 회색 티셔츠를 입고 웃었다"
       → 올바름: "승보의 카메라에 회색 티셔츠를 입은 한 여성이 담겼다. 제작진은 그녀가 오늘의 게스트라 판단했다.")
    ...
    - 위치기록: 사진은 없지만 위치 신호로만 잡힌 장면. "카메라는 놓쳤지만 제작진은 그가 ○○에 머물렀음을 안다"로 다뤄라.
